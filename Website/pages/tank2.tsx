  ...
  useEffect(() => {
    const fetchHistoricalTankData= async () => {
      const [reading2, reading3] = await Promise.all([
        fetch(`/api/fetchdata/sensor-data?board=${board_freshwater2}&sensor=${sensor_WaterLevel}&calc=${aggregation}&start=${time.start.toISOString()}&end=${time.end.toISOString()}&timeinterval=${interval}`), // HistoricWater Level
        fetch(`/api/fetchdata/sensor-data?board=${board_freshwater2}&sensor=${sensor_Temp}&calc=${aggregation}&start=${time.start.toISOString()}&end=${time.end.toISOString()}&timeinterval=${interval}`),  // Historic Temp
    ]);
...
fetchHistoricalTankData();
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/fetchdata/sensor-data?board=${board_freshwater2}&sensor=${sensor_WaterLevel}&calc=${aggregation}&timeframe=1`), // Water Level
      fetch(`/api/fetchdata/sensor-data?board=${board_freshwater2}&sensor=${sensor_Temp}&calc=${aggregation}&timeframe=1`),  // Water Temp
    ])
  ...
  function renderwaterLevelScatterPlot(data: SensorReading[] | null) {
    ...
    if (data.length === 0) return <div>No Fresh Water 2 data available.</div>;
    ...
    return (
      <PlotlyComponent
        ...
        layout={{
          ...
          yaxis: { title: "Fresh Water 2 Level (AVG)" },
          paper_bgcolor: "#f1f5f9",
          plot_bgcolor: "#f1f5f9",
        }}
        config={{ displayModeBar: false, responsive: true }}
  ...
  function renderTempScatterPlot() {
          ...
          if (waterTempHData.length === 0) return <div>No Fresh Water 2 data available.</div>;
  ...
  return (
          <div className="w-full min-h-[calc(100vh-50px)] p-2 grid grid-cols-1 gap-2 lg:grid-cols-2">
            ...
            <div className="bg-slate-100 shadow-sm rounded-md p-2">
              <h2 className="text-center text-lg sm:text-xl font-semibold font-mono">
                Historical Fresh Water 2 Tank 2
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <div className="bg-slate-100 shadow-sm rounded-md p-2 col-span-1 lg:col-span-2 h-full flex flex-col">
              ...
              <Image
                src="/Tank_Images/FreshWaterTank2NewImage.jpg"
                alt="Freshwater Tank 2 Location"
                fill

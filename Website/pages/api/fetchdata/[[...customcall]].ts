...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
    try {
        ...
        if (start && end) {
            ...
            endTime = new Date(endDateStr.includes('T') ? endDateStr : `${endDateStr}T00:00:00`);
        ...
        switch (interval) {
            case 'halfhour':
                dateFormat = '%Y-%m-%d %H:%i:00Z'; // Group by half-hour
                break;
            case 'hourly':
                dateFormat = '%Y-%m-%d %H:00:00Z'; // Group by hour
                break;
            case 'daily':
                dateFormat = '%Y-%m-%d 12:00:00Z'; // Group by day
                break;
            default:
                dateFormat = '%Y-%m-%d %H:00:00Z'; // Default to hourly
        }

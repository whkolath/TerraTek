...
const Tank1: NextPage = () => {
    ...
    useEffect(() => {
        const fetchHistoricalTankData = async () => {
            const [reading2, reading3, reading4, reading5] = await Promise.all([
                fetch(`/api/fetchdata/sensor-data?board=${board_freshwater1}&sensor=${sensor_WaterLevel}&calc=${aggregation}&start=${time.start.toISOString()}&end=${time.end.toISOString()}&timeinterval=${interval}&unit_conversion=${units}`), // HistoricWater Level
                fetch(`/api/fetchdata/sensor-data?board=${board_freshwater1}&sensor=${sensor_Temp}&calc=${aggregation}&start=${time.start.toISOString()}&end=${time.end.toISOString()}&timeinterval=${interval}&unit_conversion=${units}`),  // Historic Temp
                fetch(`/api/fetchdata/sensor-data?board=${board_freshwater1}&sensor=${sensor_pH}&calc=${aggregation}&start=${time.start.toISOString()}&end=${time.end.toISOString()}&timeinterval=${interval}&unit_conversion=${units}`),  // Historic pH
                fetch(`/api/fetchdata/sensor-data?board=${board_freshwater1}&sensor=${sensor_EC}&calc=${aggregation}&start=${time.start.toISOString()}&end=${time.end.toISOString()}&timeinterval=${interval}&unit_conversion=${units}`),  // Historic EC
            ]);
    ...
    function renderwaterLevelScatterPlot() {
        if (!waterLevelHData) return <div>Loading sensor data...</div>;
        if (waterLevelHData.length === 0) return <div>No Fresh Water 1 data available.</div>;

        const xValues = waterLevelHData.map((reading) => reading.Interval_Timestamp);
        const yValues = waterLevelHData.map((reading) => reading.Calculated_Reading);
        ...
        <div className="w-full min-h-[calc(100vh-50px)] p-2 grid grid-cols-1 gap-2 lg:grid-cols-2">
            ...
            <div className="bg-slate-100 shadow-sm rounded-md p-2 col-span-1 lg:col-span-2 h-full flex flex-col">
                ...
                <div className="relative h-64 sm:h-96 w-full flex justify-center items-center">
                    <Image
                        src="/Tank_Images/FreshWaterTank1NewImage.jpg"
                        alt="Freshwater Tank 1 Location"
                        fill
